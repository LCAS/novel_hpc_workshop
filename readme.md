...
You must be on the university network (or VPN) to access the cluster.

```bash
# 1. SSH in (use 10.10.33.11 or 10.10.33.12 if the hostname fails)
ssh <first_initial><lastname>@login.novel.hpc
```
*Note: It is highly recommended to generate per-device SSH keys (`ssh-keygen -t rsa -b 4096`) and copy them over using `ssh-copy-id` for seamless access (and VSCode integration).*
...
**To monitor your live outputs:**
Because the job is running in the background, output won't print to your terminal. Instead, you can "tail" the log files specified in your sbatch script to watch the training progress live:
```bash
tail -f logs/*
```
